...
# Returns
- `chi2_stats::Vector`: Chi2 statistics for each feature
- `p_values::Vector`: P-values for each feature
...
function _chi2(
    ...
    chi2_stats = [sum(let e = expected[i, j]; e > 0 ? (observed[i, j] - e)^2 / e : 0.0 end 
    ...
    p_values = ccdf.(Chisq(df), chi2_stats)
