...
function _fs(
    ...
    return _fs(X, nothing, featid, selector, limiter)
end
...
function _fsgroup(
    ...
    g_indices = group_id_by_aggrby(featid, aggrby)
    ...
    if group_before_score
        ...
        for (i, cur_g_indices) in enumerate(g_indices)
            ...
            grp = Tuple(Symbol.(collect(getfield(first(featid[cur_g_indices]), a) for a in aggrby)))
...
function _fsgroup(
    ...
    return _fsgroup(X, nothing, featid, selector, limiter, aggrby; kwargs...)
end
